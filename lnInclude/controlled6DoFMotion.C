...
:
    connfd(),solidBodyMotionFunction(SBMFCoeffs, runTime), socket_id(SBMFCoeffs_.get<scalar>("socket_id"))
{
...
Foam::solidBodyMotionFunctions::controlled6DoFMotion::transformation() const
{
	...
	while(std::getline(ss, token, ';')) {
		...
		switch(i){
			...
			case 6:	rotation.z() = stof(token);
				break;
			case 7:	centre_of_gravity.x() = stof(token);
				break;
			case 8:	centre_of_gravity.y() = stof(token);
				break;
			case 9:	centre_of_gravity.z() = stof(token);
				break;
		}
	}
	Info << "\nDisplacement: " << displacement;
	Info << "\nRotation: " << rotation;
    Info << "\nCofG: " << centre_of_gravity;
    ...
    septernion TR(septernion(-centre_of_gravity + -TRV[0])*R*septernion(centre_of_gravity));
